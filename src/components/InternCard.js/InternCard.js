		...
		<div className="w-full p-5 relative ring-1 bg-white ring-gray-300 ">
			...
			<div className="flex justify-between items-center mt-6  ">
				<div className="flex items-center w-1/2 justify-between ">
					<NotifyText text="1 week ago" time={true} />
					<NotifyText text="Part time allowed" />
...
const NotifyText = ({text, time}) => (
	...
	>
		<h1 className={`${time ? "text-sm text-sky-500" : "text-sm"}`}>
			{text}
		</h1>
	</div>
);
...
const DurationComp = ({text, icon, none, price}) => (
	<div className="flex items-center">
		...
		{price ? (
			...
			<h1 className={`${none ? "" : " ml-3 -mt-1 text-gray-500"}`}>
				{text}
			</h1>
		)}
	</div>
